...
import com.xiaoxiao.jackutils.info.DateTimeInfo
import java.io.*
...
class SoftUtils {
    ...
    companion object {
        @Volatile
        private var sInstance: SoftUtils? = null
        var isLog:Boolean=true
        var isFile:Boolean=true
        var LOG_TAG="SoftUtils"
        var mtag: String = "SoftUtils"
        private var curFilePath: String? = null
        var log_file_max_size = (1024 * 1024).toLong()
        private var isFileWroking:Boolean=false
        private var logFileThread:Thread?=null
        private var fileHandler: Handler?=null
        fun startFileLog(){
            if(!isFileWroking){
                logFileThread=Thread(object:Runnable{
                    override fun run() {
                        Looper.prepare()
                        fileHandler= Looper.myLooper()?.let {
                            Handler(it,object :Handler.Callback{
                                override fun handleMessage(p0: Message): Boolean {
                                    var finalData:String= p0.obj as String
                                    Log.i("file","finalData=$finalData")
                                    if (curFilePath == null) {
                                        var timesteamp=SoftUtils.getLogSteamp()
                                        curFilePath = (SoftFileSystem.getInstance()!!.getMonitorLogFilePath() + "/" +timesteamp ).toString() + ".log"
                                        ChatUtilCreateFile(curFilePath)
                                    }
                                    var curFileSize: Long = 0
                                    curFileSize = SoftUtils.ChatUtilGetFileSize(curFilePath)
                                    if (curFileSize >= log_file_max_size) {
                                        var timesteamp=SoftUtils.getLogSteamp()
                                        curFilePath = (SoftFileSystem.getInstance()!!.getMonitorLogFilePath() + "/" +timesteamp).toString()+".log"
                                        ChatUtilCreateFile(curFilePath)
                                    }
                                    SoftUtils.ChatUtilSaveDataToFileAtEnd(finalData.toByteArray(), curFilePath)
                                    return true
                                }
                            })
                        }
        ...
        fun sendFileMsg(finData:String){
            var msg=Message()
            msg.what=1
            msg.obj=finData
        ...
        fun log(tag:String,data:String) {
            if(isLog) {
                if(isFile&&isFileWroking){
                    var finalData=SoftUtils.getLogSteamp()+" "+tag+":"+data+"\n"
                    sendFileMsg(finalData)
                }
        ...
        fun getInstance(): SoftUtils? {
            if (sInstance == null) {
                synchronized(SoftUtils::class.java) {
                    if (sInstance == null) {
                        sInstance = SoftUtils()
                    }
                }
        ...
        fun UtilGetDate(): String? {
            ...
            SoftUtils.log(LOG_TAG, "UtilGetDate=$str")
            return str
        }
        ...
        fun parseTimesteamp(timestemp: String): DateTimeInfo? {
            ...
            SoftUtils.log(LOG_TAG, "parseTimesteamp,daystr=$daystr")
            ...
            SoftUtils.log(LOG_TAG, "parseTimesteamp,minstr=$minstr")
            val secstr = timestemp.substring(12, 14)
            SoftUtils.log(LOG_TAG, "parseTimesteamp,secstr=$secstr")
        ...
        fun createOneSteamp(): String? {
            ...
            val curDate = Date(System.currentTimeMillis()) //获取当前时间
            val str = formatter.format(curDate)
            SoftUtils.log(LOG_TAG, "createOneSteamp,str=$str")
            val steamp = str.substring(0, 8) + "_" + str.substring(8, 14) + "000"
            SoftUtils.log(LOG_TAG, "createOneSteamp steamp=$steamp")
            return steamp
        }
        ...
        fun getOneSteamp(): String? {
            ...
            val curDate = Date(System.currentTimeMillis()) //获取当前时间
            val str = formatter.format(curDate)
            SoftUtils.log(LOG_TAG, "getOneSteamp,str=$str")
            val steamp = str.substring(8, 14) + "000"
            SoftUtils.log(LOG_TAG, "getOneSteamp steamp=$steamp")
            return steamp
        }
